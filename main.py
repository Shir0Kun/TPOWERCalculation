...
async def export_sorted(file: UploadFile = File(...)):
    ...
    try:
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            ...
            commission_df.to_excel(writer, sheet_name='佣金计算', index=False)
            ...
            summary_df.to_excel(writer, sheet_name='佣金汇总', index=False)
            ...
            rules_df = pd.DataFrame({
                '规则说明': [
                    '=== 排序规则 ===',
                    '1. 先按上分地方排序',
                    '2. 再按代理序号排序 (OC IN → OC OUT → 其他)',
                    '',
                    '=== 佣金计算规则 ===',
                    '第一层 (OC619): (第一层+第二层+第三层总业绩) × 5%',
                    '第二层 (OC619-01): (第二层+第三层总业绩) × 20%', 
                    '第三层 (OC619-01-01/02/03/01-01): (全部第三层业绩) × 5%',
                    '其他代理: 自己业绩 × 30%',
                    '',
                    '=== 导出时间 ===',
        ...
        filename = f"佣金计算_{timestamp}.xlsx"
...
def root():
    return {
        "message": "Excel佣金计算系统",
        "description": "上传Excel文件，自动排序并计算层级佣金",
        "endpoint": "POST /export-sorted/",
        "features": [
            "自动识别上分地方和代理序号",
            "按上分地方和代理序号智能排序",
            "计算OC619系列层级佣金",
