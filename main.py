...
def calculate_hierarchical_commission_new(merged_df):
    ...
    def get_level_base(level_name):
        ...
        if level_name in hierarchy_structure:
            for child in hierarchy_structure[level_name]["children"]:
                base_amount += get_level_base(child)
    ...
    for level_name, level_info in hierarchy_structure.items():
        ...
        base_amount = get_level_base(level_name)
...
async def export_sorted(file: UploadFile = File(...)):
    ...
    try:
        ...
        if username_col:
            merged_df = merged_df.sort_values(by=username_col)
        else:
            return {"error": "找不到用户名列进行排序"}

        # 计算新的层级佣金
        commission_results = calculate_hierarchical_commission_new(merged_df)
        ...
        with pd.ExcelWriter(output, engine='openpyxl') as writer:
            # Sheet 1: 排序后的用户数据
            merged_df.to_excel(writer, sheet_name='Sorted_User_Data', index=False)
